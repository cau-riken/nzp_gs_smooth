...
def run(in_dir, cur_dir, out_dir_images, out_dir_transforms,
    ...
    Args:
        in_dir: The folder of images to process. Images should be padded with leading zeroes
        and a starting index of 1, e.g. slice_001.tif
    ...
    # Be careful to scale by the pixel size when using both SimpleITK and
    # OpenCV functions
    def deform_image(img_in, deform_in, scale_factor, out_filename):
        spacing = img_in.GetSpacing()
        img_a = sitk.GetArrayFromImage(img_in)
        img_a = img_a.astype(np.float32)
        img_c = sitk.GetArrayFromImage(deform_in)
        m_x = np.zeros([deform_in.GetHeight(), deform_in.GetWidth()])
        ...
        for j in range(0, deform_in.GetHeight()):
            for i in range(0, deform_in.GetWidth()):
                pix = img_c[j, i]
                m_x[j, i] = float(j) + scale_factor * \
                           float(pix[1]) * (1.0 / spacing[1])
                m_y[j, i] = float(i) + scale_factor * \
                           float(pix[0]) * (1.0 / spacing[0])

        out = cv2.remap(img_a, m_y, m_x, cv2.INTER_CUBIC)
    ...
    def setup_folders(in_dir):
        ...
        names_in = subprocess.check_output(
            ["ls " + in_dir], shell=True, text=True)
    ...
    def one_pass(
        ...
        for j in range(start_index, end_index, step):
            ...
            reg_run(cur_dir +
                    '/current_iter/' +
                    image_names[j - 1], cur_dir +
                    '/current_iter/' +
                    image_names[j + 1], cur_dir +
                    '/registration_output_transform/u_', cur_dir +
                    '/registration_output_image/output_warped_image.nii', ants_thread_count)
            img_u = sitk.ReadImage(
                cur_dir +
                '/registration_output_transform/u_0Warp.nii.gz',
                sitk.sitkVectorFloat64)
            # Multiply it by half the deformation field
            img_jm1 = sitk.ReadImage(
                cur_dir + '/current_iter/' + image_names[j + step])
            # Make sure to account for pixel scale factor
            deform_image(img_jm1, img_u, 0.5, cur_dir +
                         '/i_hat_image/output.nii')
            # Register Ij to IHat
            reg_run(
                cur_dir +
                '/i_hat_image/output.nii',
                ...
                image_names[j],
                cur_dir +
                '/registration_output_transform/u_',
                cur_dir +
                '/registration_output_image/output_warped_image.nii',
                ants_thread_count)
            # Merge it with previous
            img_u_acc_new = sitk.ReadImage(
                cur_dir + '/registration_output_transform/u_0Warp.nii.gz',
                sitk.sitkVectorFloat64)
            # if t == 0:
            if current_iteration == 0:
                sitk.WriteImage(img_u_acc_new, cur_dir +
                                '/current_transforms/u' + str(j) + '_Warp.nii')
            else:
                img_u_acc = sitk.ReadImage(
                    cur_dir +
                    '/current_transforms/u' +
                    str(j) +
                    '_Warp.nii',
                    sitk.sitkVectorFloat64)
                img_u_acc_new = img_u_acc + img_u_acc_new
                sitk.WriteImage(img_u_acc_new, cur_dir +
                                '/current_transforms/u' + str(j) + '_Warp.nii')
            # Update Ij using Ij0
            img_j_orig = sitk.ReadImage(
                cur_dir + '/input_with_boundary/' + image_names[j])
            deform_image(img_j_orig, img_u_acc_new, 1.0, cur_dir +
                         '/current_iter/' + image_names[j])
        # Update boundaries
        img_b = sitk.ReadImage(cur_dir + '/current_iter/' + image_names[1])
        sitk.WriteImage(img_b, cur_dir + '/current_iter/' + image_names[0])
        img_b = sitk.ReadImage(
            cur_dir + '/current_iter/' + image_names[img_count - 2])
        sitk.WriteImage(img_b, cur_dir + '/current_iter/' +
                        image_names[img_count - 1])
    ...
    if iterations > 0:
        ...
        for i in range(0, iterations):
            print("Starting iteration " + str(i + 1) + " of " + str(iterations))
            if i % 2 == 0:
                one_pass(1, img_count - 1, 1, cur_dir, i, image_names,
                         img_count, ants_thread_count)
            else:
                one_pass(img_count - 2, 0, 1, cur_dir, i, image_names,
                         img_count, ants_thread_count)
    ...
    # Copy from the input directory first
    names_in = subprocess.check_output(
        ["ls " + out_dir_images], shell=True, text=True)
    names_in = names_in.split()
    img_count = len(names_in)
